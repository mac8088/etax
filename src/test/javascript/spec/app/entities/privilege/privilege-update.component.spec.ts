/* tslint:disable max-line-length */
import { shallowMount, createLocalVue, Wrapper } from '@vue/test-utils';
import sinon, { SinonStubbedInstance } from 'sinon';
import Router from 'vue-router';

import AlertService from '@/shared/alert/alert.service';
import * as config from '@/shared/config/config';
import PrivilegeUpdateComponent from '@/entities/privilege/privilege-update.vue';
import PrivilegeClass from '@/entities/privilege/privilege-update.component';
import PrivilegeService from '@/entities/privilege/privilege.service';

const localVue = createLocalVue();

config.initVueApp(localVue);
const i18n = config.initI18N(localVue);
const store = config.initVueXStore(localVue);
const router = new Router();
localVue.use(Router);
localVue.component('font-awesome-icon', {});

describe('Component Tests', () => {
  describe('Privilege Management Update Component', () => {
    let wrapper: Wrapper<PrivilegeClass>;
    let comp: PrivilegeClass;
    let privilegeServiceStub: SinonStubbedInstance<PrivilegeService>;

    beforeEach(() => {
      privilegeServiceStub = sinon.createStubInstance<PrivilegeService>(PrivilegeService);

      wrapper = shallowMount<PrivilegeClass>(PrivilegeUpdateComponent, {
        store,
        i18n,
        localVue,
        router,
        provide: {
          alertService: () => new AlertService(store),
          privilegeService: () => privilegeServiceStub
        }
      });
      comp = wrapper.vm;
    });

    describe('save', () => {
      it('Should call update service on save for existing entity', async () => {
        // GIVEN
        const entity = { id: 123 };
        comp.privilege = entity;
        privilegeServiceStub.update.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(privilegeServiceStub.update.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });

      it('Should call create service on save for new entity', async () => {
        // GIVEN
        const entity = {};
        comp.privilege = entity;
        privilegeServiceStub.create.resolves(entity);

        // WHEN
        comp.save();
        await comp.$nextTick();

        // THEN
        expect(privilegeServiceStub.create.calledWith(entity)).toBeTruthy();
        expect(comp.isSaving).toEqual(false);
      });
    });
  });
});
